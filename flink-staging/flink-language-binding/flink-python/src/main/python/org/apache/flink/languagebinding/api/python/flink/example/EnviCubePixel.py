...
from flink.plan.Constants import TILE, STRING, CUBE, TIMESERIES
from flink.plan.Constants import Tile, Cube, TimeSeries
...
class CubeSplitter(FlatMapFunction):
    def flat_map(self, value, collector):
        """
            does the splitting of a cube into sub cubes of a certain with
            potentially 1x1
        :param value:
        :param collector:
        :return:
...
class ApproxTimeSeries(FlatMapFunction):
    def flat_map(self, value, collector):
        """
            does the splitting of a cube into sub cubes of a certain with
            potentially 1x1
...
if __name__ == "__main__":
    ...
    subCubes = cube.map(CubeSplitter(), CUBE)
    result = subCubes.map(ApproxTimeSeries(), TIMESERIES)
