...
public abstract class SpatialInputFormat<OT> extends FileInputFormat<OT> {

	private static final long serialVersionUID = 1L;
	private static final Logger LOG = LoggerFactory.getLogger(SpatialInputFormat.class);
	protected int xsize = -1;
	protected int ysize = -1;
	protected Coordinate leftUpperLimit = null;
	protected Coordinate rightLowerLimit = null;
	protected double pixelWidth = -1.0;
	protected double pixelHeight = -1.0;
	protected TileInfo info;
	protected EnviTilePosition pos;
	protected boolean completeScene;
	 ...
	 */
	private static final boolean dotPairsDontMix(double a1, double a2,
			double b1, double b2) {
	 ...
	 */
	protected final boolean rectIntersectsLimits(Coordinate tileUpperLeft, Coordinate tileLowerRight) {
	...
	public SpatialInputFormat(Path path) {
		super(path);
	}

	public void setLimitRectangle(Coordinate leftUpperLimit, Coordinate rightLowerLimit) {
	...
	private void createTiledSplits(FileSystem fs, List<FileInputSplit> inputSplits, FileStatus file,
			TileInfo info, Path dataFile, FileStatus dataFileStatus, int data_size, int numBands,
			int numRows, int numColumns, String pathRow, String aqcDate)
			throws IOException {
			/*
